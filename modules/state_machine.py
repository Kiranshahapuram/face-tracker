...
class FaceStateMachine:
    ...
    def bind_face_id(self, track_id, face_id, embedding, is_new, frame_number, frame, db, logger, session_id, io_queue):
        events = []
    ...
    def _fire_entry(self, state_obj, track_id, frame_number, frame,
        ...
        if crop.size > 0:
            io_queue.put({'type': 'save_image', 'path': img_path, 'crop': crop})
        io_queue.put({
            'type':         'log_event',
